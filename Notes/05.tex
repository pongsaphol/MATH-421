...
want to show $\displaystyle\int_{2^{-N}}^1 \frac{1}{t}\,\mathrm{d}t \to \infty$ if $N \to \infty$
...
\[ \frac{A(c+h) - A(c)}{h} = \frac{1}{h} \left(-\int_{c+h}^c f(t)\,\mathrm{d}t\right) = \frac{1}{|h|}\int_{c-|h|}^{c}f(t)\,\mathrm{d}t\]
